...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* Mobile Drawer Panel Backdrop (Darkened, high-blur, high z-index) */}
      {isOpen && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-md z-[999] lg:hidden transition-opacity"
          onClick={() => setIsOpen(false)}
        />
      )}

      {/* Mobile Drawer Left Panel Menu (Solid background, extremely high z-index) */}
      <div
        className={`fixed inset-y-0 left-0 w-64 bg-[var(--bg-base)] border-r border-[var(--line)] p-6 z-[1000] transform transition-transform duration-300 ease-in-out lg:hidden ${
